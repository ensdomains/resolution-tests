...
import (
	...
	ens "github.com/ensdomains/go-ens/v4"
...
func loadReadyCases(category string) []testCase {
	...
	out := make([]testCase, 0)
	for _, c := range cases {
		if c.Status != "ready" || c.Category != category {
			continue
		}
		out = append(out, c)
	}
...
// ensip11CoinType converts an EVM chain ID to an ENSIP-11 coin type.
func ensip11CoinType(chainID uint64) uint64 {
...
func runForward(ctx context.Context, client *ethclient.Client, ur *ens.UniversalResolver, c testCase) (*string, error) {
	switch c.Method {
	case "addr":
		if c.Input.Name == nil {
		...
		if coinType == ens.CoinTypeETH {
			addr, err := ens.Resolve(ctx, client, *c.Input.Name)
			if err != nil {
				return nil, err
			}
			s := addr.Hex()
		...
		raw, err := ur.MultiAddress(ctx, *c.Input.Name, coinType)
		if err != nil {
			return nil, err
		}
		if len(raw) == 0 {
	...
	case "text":
		...
		value, err := ur.Text(ctx, *c.Input.Name, key)
		if err != nil {
			return nil, err
		}
		if value == "" {
	...
	case "contenthash":
		...
		raw, err := ur.Contenthash(ctx, *c.Input.Name)
		if err != nil {
			return nil, err
		}
		if len(raw) == 0 {
...
func runReverse(ctx context.Context, client *ethclient.Client, ur *ens.UniversalResolver, c testCase) (*string, error) {
	switch c.Method {
	case "reverse":
		...
		name, err := ens.ReverseResolve(ctx, client, addr)
		if err != nil {
			return nil, err
		}
		if name == "" {
	...
	case "reverse-l2":
		...
		chainID, ok := paramUint64(c.Params, "chainId")
		if !ok {
			return nil, fmt.Errorf("missing chainId")
		}
		addr := common.HexToAddress(*c.Input.Address)
		name, _, _, err := ur.Reverse(ctx, addr.Bytes(), ensip11CoinType(chainID))
		if err != nil {
			return nil, err
		}
		if name == "" {
...
func TestForwardResolution(t *testing.T) {
	...
	ur, err := ens.NewUniversalResolver(ctx, client)
	if err != nil {
		t.Fatalf("universal resolver: %v", err)
	}

	var failures []string
	for _, c := range loadReadyCases("forward") {
		start := time.Now()
		actual, err := runForward(ctx, client, ur, c)
...
func TestReverseResolution(t *testing.T) {
	...
	for _, c := range loadReadyCases("reverse") {
		start := time.Now()
		actual, err := runReverse(ctx, client, ur, c)
		duration := time.Since(start).Milliseconds()
		expected := expectedValue(c)
