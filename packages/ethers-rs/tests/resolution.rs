//! ENS resolution tests for ethers-rs (`Middleware` ENS helpers).
...
/// Public ethers-rs ENS helpers do not cover these methods today.
    ...
    cases
        ...
        .filter(|c| {
            c.status == "ready"
                ...
                // ethers-rs resolve_name only resolves ETH (coin type 60)
                && !(c.method == "addr"
                    && c.params
                        .get("coinType")
                        .and_then(|v| v.as_u64())
...
fn provider() -> Result<Provider<Http>, String> {
    Provider::<Http>::try_from(load_rpc_url()).map_err(|e| e.to_string())
}
